    ...
    for (clust in as.character(unique(markers[[candidate]]$cluster))) {
      if (length(genes.candidate) == 0) {
        ...
        data <- data.frame(as.factor(sobj@meta.data[[candidate]] == 
                                       clust), as.numeric(sobj@assays$RNA@data[genes.candidate, 
                                       ]))
        colnames(data) <- c("label", genes.candidate)
      }
      else {
        data <- as.data.frame(t(as.matrix(sobj@assays$RNA@data[genes.candidate, 
        ])))
...
ikap2 <- function (sobj, pcs = NA, pc.range = 20, k.max = NA, r.kmax.est = 1.5, 
